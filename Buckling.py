...
class MarginOfSafety:
    ...
    def find_mos(self):
        ...
        max_stress_normal = NormalStressCalcs("Combined", self.n_str_top, self.n_str_bot, self.self.width_str,
                                              self.area_str, self.centroid_x, self.centroid_y, self.th_spar, self.th_flang,
                                              self.height_str, self.thick, self.corner_coords[0], self.corner_coords[3]).find_stress_at_span(self.span_position)[1]
...
class BuckleColumn:
    ...
    crit_buckle_stringer_vec = np.vectorize(crit_buckle_stringer)
